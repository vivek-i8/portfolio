...
const PreloadContext = React.createContext<{ isPreloading: boolean, phase: PreloadPhase }>({ isPreloading: true, phase: "intro" });
...
export function ArcRevealHero({
  ...
  if (pathname !== prevPathname) {
    ...
    // Skip the preloader when returning from a project detail page to the listing
    if (!isBackToProjects) {
        setPhase("intro");
        setIndex(0);
        progress.set(0);
  ...
  React.useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentionally mount-only: checks initial session state once
  }, []);
