...
import { X, Calendar, Code, Box, Share2, ExternalLink, Github, Terminal, CheckCircle2, Copy, Check, Maximize2, Zap, Cpu, Info } from 'lucide-react';
    ...
    return (
        <div className="flex flex-col gap-12 pb-12">
            {images.map((img, idx) => (
                ...
                >
                    {/* eslint-disable-next-line @next/next/no-img-element -- gallery images with natural h-auto aspect ratios from mixed sources */}
                    <img
                        src={img}
...
export function ProjectDetail({ project, onClose, isLowPowerMode }: { project: Project; onClose: () => void; isLowPowerMode?: boolean }) {
    ...
    // Lenis keeps scrolling its own way, so the body has to be frozen too.
    useEffect(() => {
        if (lenis) lenis.stop();
    ...
    return (
        ...
        >
            ...
            >
                ...
                <motion.button
                    onClick={() => {
                        // Unlock before the exit animation starts, otherwise heavy tabs stall the restore.
                        document.documentElement.style.overflow = '';
                        document.body.style.overflow = '';
                        if (lenis) lenis.start();
                        onClose();
