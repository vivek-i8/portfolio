...
// Abstract editorial vector illustrations for soft skills (low opacity atmospheric imagery)
const skillVisuals: Record<string, string> = {
  'Problem Solving': '/skills/problem-solving.svg',
  'Systemic Thinking': '/skills/systemic-thinking.svg',
  'Critical Thinking': '/skills/critical-thinking.svg',
  'Continuous Learning': '/skills/continuous-learning.svg',
  'Analytical Thinking': '/skills/analytical-thinking.svg',
  'Adaptability': '/skills/adaptability.svg',
  'Leadership': '/skills/leadership.svg',
  'Communication': '/skills/communication.svg',
  'Teamwork': '/skills/teamwork.svg',
  'Research Skills': '/skills/research-skills.svg',
};
...
const allCards = portfolioData.softSkills.slice(0, 10).map((skill, index) => ({
  ...
  url: skillVisuals[skill.name] || '/skills/systemic-thinking.svg',
  Icon: skillIcons[skill.name] || Users
}));

export const SoftSkillsCarousel = () => {
        ...
        <div className="w-full px-6 md:px-24 pt-6 md:pt-8 lg:pt-12 z-20 flex-shrink-0">
          ...
          >
            ...
            <p className="text-muted-foreground text-base md:text-lg lg:text-xl font-medium max-w-2xl leading-relaxed">
              Analytical disciplines and collaborative methodologies engineered for complex problem solving and resilient system architecture.
            </p>
          </motion.div>
        </div>
...
const Card = ({ card }: { card: typeof allCards[0] }) => {
  ...
  return (
    ...
    >
      ...
      {/* Abstract vector imagery with increased visibility */}
      <div className="absolute inset-0 z-0 flex items-center justify-center p-6 sm:p-8 transition-transform duration-700 group-hover:scale-[1.03] opacity-55 group-hover:opacity-75">
        {/* eslint-disable-next-line @next/next/no-img-element -- decorative local SVG illustrations, no optimization benefit */}
        <img
          src={card.url}
          alt={card.title}
      ...
      <div className="absolute inset-0 z-10 bg-gradient-to-t from-background/95 via-background/40 to-transparent transition-opacity duration-500 pointer-events-none"></div>
